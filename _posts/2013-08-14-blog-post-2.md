...
![](/images/CES_logo_rgb.png){: width="250" }
...
We'll also be using the `dataverse` and `survey` packages in this tutorial. I will not explain them in detail here. I have discussed them (and provided additional resources) in my last post: [_Plotting trends over time with the CES_](https://pdeshlab.github.io/posts/CES-plots-over-time). 

Much of this tutorial uses code that is adapted from an [article](https://www.cgoodman.com/blog/archives/2018/06/16/maps-in-r-using-urbnmapr/) by Christopher Goodman and the Urban Institute's very own [tutorial](https://urbaninstitute.github.io/r-at-urban/mapping.html#Introduction). When I do this adapting, I note it in-text, but I wanted to give credit up-top too. Thank you for these incredible
resources!
...
```{r whotookit_sequential, include = TRUE}
# Plotting 
# Code adapted from: 
...
* I am genuinely interested in answering a _geographic_ question, and a map would help.
...
Oh, you thought we were done? Well I learned something cool recently, so I wanted to share it with y'all! What if your maps were. . . interactive?

Besides interactive things just being fun, they're a great way to let your readers explore your data. In fact, they're a great way for you to explore your data! Lucky for us the `leaflet` package has us covered. We are also going to rely on OpenStreetMap tiles (which are open source) as opposed to other---expensive---options.
...
leaflet(states) %>%
  ...
  saveWidget("baseleaflet.html") # save map widget
                                 # this step is only important to 
                                 # helping us render this map on the web

```

This is moderately exciting. You can click and drag this map! You can zoom in and out! But now what? Before we go any further, we need some data. I find `leaflet` maps work best when you have data that includes latitude and longitude. Surveys will not give you the latitude and longitude of respondents because this information may make it easy for someone to find who a particular respondent is, which is a violation of their privacy. However, there are many other contexts where interactive maps will be useful, especially when studying institutions. If you find a very interesting public opinion trend using the CES or other large survey, the next step is to try and figure out why that may be! 

Interactive maps are also excellent at letting readers see how larger data trends may apply to their smaller communities, a particularly important job for journalists. For example, _The Washington Post_ conducted a [detailed investigation](https://www.washingtonpost.com/graphics/2019/investigations/pharmacies-pain-pill-map/) into the sales of addictive opiods in the United States. This map lets readers search their address and see how many opioids their local pharmacy sold. You can also see national trends and how your community compares to others.

What important question am I going to ask with my interactive map? Well, I have a friend from Seattle who has never heard of Waffle House, and I want to know why. I bet that mapping the locations of all the stores may help, so let's load in some data! Our dataset must have latitude and longitude coordinates. Many datasets you will encounter (or create yourself) will only include addresses, which you can use to find lat and lon coordinates yourself, in a process called *geocoding*. Geocoding can be done with proprietary software like GIS, but can also be accomplished with open source tools and packages through Python or R. I won't go into how to geocode in this tutorial, but I would encourage you to look into it if it piques your interest.

Back to Waffle House: this godsend of a [dataset](https://github.com/minh5/wafflehouse/blob/master/webscraping/geoWaffles.csv) was published by Minh Mai. I will link to to Mai's GitHub [here](https://github.com/minh5).

```{r waffle, include = TRUE}
...
I am downloading a csv here to avoid explaining a new file format, but `leaflet` maps (and other spatial mapping tools) often use geoJSONs. [Here](https://geojson.org/) is a brief explanation of the file format. You are likely to find datasets in this format, so it is good to try and understand this particular data type! But for now, let's stick with our waffle csv.

```{r leaflet_2, include = TRUE}
...
leaflet(waffle) %>%
  ...
  addTiles() %>%
  addProviderTiles(providers$CartoDB.Positron) %>% # add muted palette basemap
  saveWidget(here::here("waffle.html")) # save map widget

```
Ah! Now I have a good working theory. If you're from Seattle, you may have never seen a Waffle House before. You've never had that cursed experience and your life is less complete for it. (Of course, since my location dataset has not been updated since 2015, this may have changed. Perhaps I should try to collect more up to date data and run this analysis again!)

Plotting the locations of important buildings, businesses, and people can help us answer questions about access to resources--- whether that is to polling places, primary schools, or Waffle Houses.

That brings us to the end of our tutorial. As always, the whole script is below (slightly rearranged so it will hopefully run smoothly on your computer). One note: you need to download the Waffle House locations onto your PC if you'd like to load that graph, without it, that section of the code will break!
